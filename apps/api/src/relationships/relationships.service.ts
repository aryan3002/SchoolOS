...
export class RelationshipsService {
  ...
  async findMany(options: RelationshipQueryOptions): Promise<{
    ...
    if (userId) {
      where.OR = [{ userId: userId }, { relatedUserId: userId }];
    }
  ...
  async create(input: CreateRelationshipInput): Promise<RelationshipRecord> {
    ...
    const [primaryUser, relatedUser] = await Promise.all([
      this.prisma.user.findFirst({
        where: { id: input.userId, districtId: input.districtId },
      }),
      this.prisma.user.findFirst({
    ...
    // Invalidate cache
    await this.invalidateCacheForUsers([input.userId, input.relatedUserId]);
  ...
  async update(
    ...
    const data: Prisma.UserRelationshipUpdateInput = {};
    ...
    for (const input of relationships) {
      try {
        ...
        if (error instanceof ConflictException) {
          ...
          errors.push(
            `Failed to create relationship ${input.userId} -> ${input.relatedUserId}: ${(error as Error).message}`,
          );
        }
      }
    }
