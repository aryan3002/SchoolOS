...
const clean = (value) => value ? value.trim() : undefined;
class SkywardMapper {
    mapStudent(payload) {
        ...
        return {
            ...
            studentNumber: String(student.studentNumber ?? student.StudentNumber ?? student.StudentID),
            ...
            preferredName: clean(student.preferredName ?? student.NickName ?? student.PreferredName),
        ...
        return {
            ...
            exitDate: enrollment.exitDate
                ...
                : enrollment.ExitDate
                    ? toDate(enrollment.ExitDate)
                    : undefined,
            schoolYear: String(enrollment.schoolYear ??
    ...
    mapGuardian(payload) {
        ...
        return {
            ...
            relationship: clean(guardian.relationship ?? guardian.Relationship) ?? 'guardian',
            ...
            receiveCommunications: Boolean(guardian.ReceiveCommunications ?? guardian.receiveCommunications ?? true),
            metadata: guardian,
        };
    }
    mapContact(payload) {
    ...
    mapAttendance(payload) {
        ...
        return {
            ...
            status: this.mapAttendanceCode(attendance.Status ?? attendance.status ?? attendance.Code),
    ...
    mapStatus(status) {
        ...
        if (normalized.includes('inactive'))
            return 'inactive';
        if (normalized.includes('grad'))
            return 'graduated';
        if (normalized.includes('transfer'))
            return 'transferred';
        return 'active';
    }
    mapAttendanceCode(code) {
...
exports.SkywardMapper = SkywardMapper;
//# sourceMappingURL=skyward.mapper.js.map