...
exports.SkywardConnector = void 0;
const client_1 = require("@prisma/client");
const skyward_client_1 = require("./skyward.client");
const skyward_mapper_1 = require("./skyward.mapper");
/**
 * Skyward Connector implementation.
 */
class SkywardConnector {
    credentials;
    prisma;
    logger;
    client;
    ...
    constructor(credentials, prisma, logger) {
        ...
        this.prisma = prisma;
        this.logger = logger;
        this.mapper = new skyward_mapper_1.SkywardMapper();
        this.client = new skyward_client_1.SkywardClient(credentials, logger.child ? logger.child({ vendor: 'skyward' }) : logger);
    }
    async authenticate(credentials) {
        this.credentials = credentials;
        this.client = new skyward_client_1.SkywardClient(credentials, this.logger.child ? this.logger.child({ vendor: 'skyward' }) : this.logger);
    ...
    async syncDistrict(districtId, options) {
        ...
        const errors = [];
        const campuses = options.schoolIds && options.schoolIds.length > 0
...
exports.SkywardConnector = SkywardConnector;
//# sourceMappingURL=skyward.connector.js.map